...
- The phase 0 datasets from gsm.safety#77 (`data/FDA_AbnormalityLevels.rda`, `data/FDA_ExtremeValues.rda`, documented in `R/data.R`): `FDA_AbnormalityLevels` (Basis values absolute, uln_multiple, baseline_multiple, baseline_percent_decrease, baseline_change) and `FDA_ExtremeValues` (one row per parameter per UnitSystem, columns Unit, Low, High), and the derivations `Derive_ULNMultiple()`, `Derive_AbnormalityLevel()`, `Derive_ExtremeValueFlag()` (gsm.safety#78) that run over adbds-shaped long results.
...
| `PARAM` | every lab figure; parameter lookup in `FDA_AbnormalityLevels` and `FDA_ExtremeValues` | `adbds$TEST` (23 lab tests, names such as "Alanine Aminotransferase", "Bilirubin") | `Mapped_LB$lbtstnam` (LB.yaml; saf0001.yaml) | found | The contract keys on the test name, as `Input_HysLaw()` already does through `lMeasureValues`. The lookup from test name to FDA parameter is the `lParameterValues` argument of each `Derive_*` function, whose default covers the adbds names. |
| `PARAMCD` | none of the figures; convenient for lookups | not in `adbds` (`adeg` has it) | none | derived | Derive from the test name through the same `lParameterValues` map. Not a contract column. |
